...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className={`flex h-screen bg-[#FAFAFB] text-zinc-800 ${montserrat.className} selection:bg-zinc-200 selection:text-black overflow-hidden relative`}>
      
      {/* MAGIA DE FONDO */}
      <div className="absolute top-[20%] -left-[10%] w-[60vw] h-[60vw] bg-white rounded-full blur-[100px] pointer-events-none z-0" />
      
      <div className="absolute bottom-[10%] -right-[5%] w-[80vw] h-[80vw] opacity-[0.03] pointer-events-none z-0 text-zinc-600">
        <TetragrammatonIcon className="w-full h-full animate-[spin_240s_linear_infinite]" />
      </div>

      {/* SIDEBAR DE CRISTAL PLATA */}
      <aside className="hidden md:flex flex-col w-76 bg-white/70 backdrop-blur-3xl border-r border-zinc-200/70 shadow-[10px_0_40px_rgba(0,0,0,0.02)] z-20 relative">
        <div className="absolute inset-y-0 right-0 w-px bg-gradient-to-b from-transparent via-zinc-300 to-transparent opacity-50" />
        
        <div className="p-10 relative overflow-hidden flex flex-col items-center text-center">
          <div className="relative w-16 h-16 mb-6 rounded-full flex items-center justify-center bg-gradient-to-b from-white to-zinc-50 border-silver-real shadow-md">
            <TetragrammatonIcon className="w-8 h-8 text-zinc-400" />
            <div className="absolute inset-0 bg-white/40 rounded-full animate-pulse blur-sm -z-10" />
          </div>
          <h2 className={`text-2xl font-semibold text-silver-shimmer uppercase tracking-widest ${cormorant.className}`}>
            Velo Interno
          </h2>
          <p className="text-[9px] text-zinc-400 mt-2 font-black tracking-[0.4em] uppercase">Control Energético</p>
        </div>
        
        <nav className="flex-1 px-6 space-y-4 mt-6 z-10">
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.name}
                href={item.href}
                className={`group flex items-center gap-4 px-5 py-4 rounded-[1.5rem] transition-all duration-500 relative overflow-hidden ${
                  isActive 
                    ? 'bg-white shadow-[0_5px_15px_rgba(0,0,0,0.04),inset_0_1px_1px_rgba(255,255,255,1)] border border-zinc-200/80 text-zinc-900' 
                    : 'border border-transparent text-zinc-500 hover:text-zinc-800 hover:bg-white/50 hover:shadow-sm'
                }`}
              >
                {/* Indicador activo sutil */}
                {isActive && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-zinc-300 rounded-r-full shadow-[0_0_10px_rgba(212,212,216,0.8)]" />}
                
                <div className={`p-2 rounded-xl transition-all duration-500 ${isActive ? 'bg-zinc-100/80 shadow-inner' : 'bg-transparent group-hover:scale-110'}`}>
                  <item.icon className={`w-5 h-5 transition-transform duration-500 ${isActive ? 'text-zinc-800' : 'text-zinc-400 group-hover:text-zinc-600'}`} />
                </div>
                <span className="text-[11px] tracking-[0.15em] uppercase font-bold">{item.name}</span>
              </Link>
            );
          })}
        </nav>
      ...
      {/* MOBILE NAV (Cristal Plata) */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-2xl border-t border-zinc-200 flex justify-around items-center h-20 px-2 z-50 pb-safe shadow-[0_-10px_30px_rgba(0,0,0,0.03)]">
        {navItems.map((item) => {
          const isActive = pathname === item.href || (pathname.startsWith(item.href) && item.href !== '/dashboard');
          return (
            <Link key={item.name} href={item.href} className="flex flex-col items-center justify-center min-w-[70px] h-full space-y-1.5 active:scale-95">
              <div className={`p-2 rounded-full ${isActive ? 'bg-zinc-100 shadow-inner' : ''}`}>
...
// Iconos (Mismos de tu versión anterior)
